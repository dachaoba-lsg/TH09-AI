...
local function buildCandidates(player, cfg, model, stats)
  local candidates = {}
...
local function scoreObject(player, object, kind, candidates, cfg, stats, previous_lasers, next_lasers, frame)
  ...
  if max(center_x, end_x) + bound_x < player.x - reach
    or min(center_x, end_x) - bound_x > player.x + reach
    or max(center_y, end_y) + bound_y < player.y - reach
    or min(center_y, end_y) - bound_y > player.y + reach then return end
  stats.objects_relevant = stats.objects_relevant + 1
  if warning then stats.warning_lasers = stats.warning_lasers + 1 end
  for _, candidate in ipairs(candidates) do
    for _, segment in ipairs(candidate.segments) do
...
local function choose(game_side, state, cfg)
  ...
  local candidates = buildCandidates(player, cfg, model, stats)

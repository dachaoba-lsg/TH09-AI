...
# Unicode names are built from code points so this script also runs in
# Windows PowerShell 5.1 when read without a UTF-8 BOM.
...
try {
    foreach ($file in $packageFiles) {
        [IO.Compression.ZipFileExtensions]::CreateEntryFromFile($archive, $file.File,
            $PackageName + '/' + $file.Relative, [IO.Compression.CompressionLevel]::Optimal) | Out-Null
    }
} finally {
    $archive.Dispose()
    $stream.Dispose()

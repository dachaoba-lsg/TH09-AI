...
if baseline_file then
  ...
  -- 3.0 adds a human-like movement cap that intentionally changes long
  -- sequences of direction choices, so this optional baseline audit disables
  -- it and keeps comparing only the height/optional-min_y behaviour.
  local audit = config()
  ...
  for frame = 1, 300 do
    ...
    for variant = 1, 2 do
      ...
      local a = old.choose(w, old_states[variant], audit, intent)
      local b = dodge.choose(w, new_states[variant], audit, intent)

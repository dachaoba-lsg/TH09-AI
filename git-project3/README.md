...
《东方花映塚》日文版 v1.50a 的本地 AI 源码（默认接管 2P）。Lua 负责决策，本项目的原生启动器和支持模块提供输入隔离、窗口处理及只读感知，运行时使用上游 ka_ai_duka v1.7。
...
在 Match Mode → Human vs Human 中控制实际选中的一侧；该侧的 `Charge Type` 必须为 `Slow`，AI 不使用 X。保留连爆资源判断、C2 循环及受真实能量与动作门约束的压力反馈，不承诺必放 C、纯开花或固定存活时间。`seconds` 顶层配置是接管时限，默认 600，0 为不限时，与难度预算独立。

...
## Opponent gauge read-only extension (3.0 step 1)

The core player.sensor apiVersion remains 1. opponentApiVersion=1 adds a nested
`opponent` table for the OTHER board. Humankind reads the opponent gauge before
choosing a charge level, so 3.0 bloom decisions need it; this extension only
reads, and a failure never invalidates the own snapshot.

| Field | Meaning |
| --- | --- |
| valid | True only when every opponent field below was read and range-checked. False is a normal state between rounds. Invalid entries are exported as zero, never as "no energy". |
| chargeCurrent, chargeMax | Opponent's `charge_current`/`charge_max` (0..400 verified range), the same quantities the local HUD draws. |
| chargeSpeed | Opponent feature charge speed (0..100). Zero means unread/not loaded and must not be treated as instant charge. |
| state | Raw opponent player state (0..5). |
| protectionFrames | Opponent state-3 remaining protection, otherwise zero. |
| life, spellPoint, combo | Opponent life, spell points and current combo, for diagnosis and future C3/gauge comparison. |

The board array at 0x4a7d94 already proved the hooked player's identity for the
own snapshot; the opponent entry is that same array's other player pointer. Any
out-of-range state, gauge, speed, life, combo or spell point, an unaligned
pointer or a failed read only clears the `opponent` sub-snapshot. Lua treats a
missing `opponentApiVersion` as an older native build (still acceptable) and a
present but malformed sub-table as a deployment fault that releases AI input.

...
function M.observe(game_side, state, cfg)
  local c, result = settings(cfg), {
    valid = false, counts = { fairies = 0, spirits = 0, activated = 0, erasable = 0, field_erasable = 0,
      field_speed_sum = 0, field_speed_count = 0, field_erasable_speed = 0 },
  ...
  for _, b in ipairs(bullets) do
    ...
    if validObject(b) then
      if b.isErasable == true then
        -- Whole-field erasable bullets, no near-player filter. This is the
        -- 3.0 bloom pressure reading; it is a count of observed objects, not a
        -- promise that a released circle reaches all of them. The mean speed of
        -- those bullets is an observed rank proxy: the game rank itself is not
        -- readable and differs per player, so only the local field is used.
        if inField(b.x, b.y) then
          result.counts.field_erasable = result.counts.field_erasable + 1
          local vx, vy = b.vx, b.vy
          if finite(vx) and finite(vy) then
            result.counts.field_speed_sum = result.counts.field_speed_sum
              + math.sqrt(vx * vx + vy * vy)
            result.counts.field_speed_count = result.counts.field_speed_count + 1
          end
        end
  ...
  -- velocity sample", which the bloom policy must treat as unknown, not as slow.
  if result.counts.field_speed_count > 0 then
    result.counts.field_erasable_speed =
      result.counts.field_speed_sum / result.counts.field_speed_count
  end
  return result
end

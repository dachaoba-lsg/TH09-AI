# DS-TH09-AI 3.5.0-test：灵梦 C1 修复

基于已验收的 3.4：灵梦 C1 增加目标血量、固灵状态、非贯穿伤害与追踪路径估计，蓄力中复核原起爆目标；只读 enemy sensor 与 player sensor 提供同一对象的战斗元数据。期限型 C1、C2 循环、圆形视野和四项能力预算保留。预测不保证实战必定击杀；出屏销毁和动画到期尚未完整建模。

AI 仍默认 2P。请求 1P 必须通过 `ai.side_key` 本地校验，缺失或错误回退 2P；公开配置为空 key，源码只含摘要。实际 AI 侧输入独占、Slow 要求和 1P 练习保护抑制随有效侧生效。键位不变，CSV 为169列，旧162列前缀保留。

完整设置见 [玩家说明](../package/README.md)，可重复构建和测试见 [BUILDING](../BUILDING.md)。此前“达到50W按压力断分”仍非当前已实现状态机；不能将短暂停射等同于该功能。

---

以下为历史版本记录，其参数与说明以各历史版本为准。

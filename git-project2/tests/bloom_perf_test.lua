...
local function pipeline(w, state)
  state.frame = state.frame + 1
  state.observer.lock_ids = state.bloom.target_level and state.bloom.attack_ids or nil
  local observer_cfg = {}
  for k, v in pairs(cfg.bloom.observer) do observer_cfg[k] = v end
  ...
  local plan = bloom.update(w, state.bloom, cfg.bloom, obs)
  local planned = now()
  local movement = dodge.choose(w, state, cfg.dodge, plan.intent)
  local moved = now()
  bloom.feedback(state.bloom, movement, cfg.bloom)
  local finished = now()
  ...
  assert(movement.sensor_valid == 1 and movement.objects_seen == #w.enemies + #w.bullets + #w.exAttacks,
    "dodge bypassed valid sensing or dropped scene entries")
...
local function benchmark(name, w)
  local state = { frame = 0, bloom = {}, observer = {} }
  for _ = 1, 3 do pipeline(w, state) end
  ...
  for _ = 1, iterations do
    ...
    elapsed, obs, plan, movement = pipeline(w, state)
  ...
  print(string.format("%s,%d,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%d/%d/%d/%d,%d,%d,%d,%d,%d,%s",
    ...
    movement.movement_segments, plan.phase))
end
...
print("scenario,enemies,bullets,iterations,observe_ms,policy_ms,dodge_ms,feedback_ms,total_ms,peak_ms,stage_calls_o_p_d_f,enemy_pairs,seen,relevant,trajectories,segments,last_phase")
...
print("bloom_perf_test: PASS (7 synthetic scenes; real observer/policy/dodge/feedback on every iteration; no X or C3/C4 plan)")

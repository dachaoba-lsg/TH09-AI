"""Real upstream SetPlayerFields / injected bridge, in isolated x86 Unicorn.
...
player_sensor_selftest.exe. Requires pefile + unicorn (work/input-fix/python
...
def execute(image, base, plan, patched, prior_tables, helper_noop=False, enemy=False):
    ...
    put(OBJECT, RAW); put(OBJECT + 4, CHARACTER); put(CHARACTER, 12)
    put(RAW + 0x30338, FEATURE)
    f(RAW + 0x1B88, 17.5); f(RAW + 0x1B8C, 300.25)
    for offset, value in ((0xA8, 10), (0xA0, 6), (0xA4, 7), (0x30414, 8), (0x3041C, 420000)):
        put(RAW + offset, value)
    for offset, value in ((0x30384, 155.5), (0x30388, 301.25)):
        f(RAW + offset, value)
...
def main():
    ...
    for enemy, base in ((e, b) for e in (False, True) for b in (0x10000000, 0x14000000, 0x65000000)):
        ...
        for prior in (False, True):
            ...
            cases += 1
    print(f"native_player_sensor_test: PASS ({cases} real upstream code / relocated ABI cases)")

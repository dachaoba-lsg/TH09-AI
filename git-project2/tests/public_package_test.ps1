...
try {
    ...
    $runtimeLua = @($zip.Entries | Where-Object { $_.FullName -match '^TH09-AI/ai/[^/]+\.lua$' } |
        ForEach-Object { $_.FullName.Substring('TH09-AI/ai/'.Length) } | Sort-Object)
    $sourceLua = @($zip.Entries | Where-Object { $_.FullName -match '^TH09-AI/source/src/ai/[^/]+\.lua$' } |
        ForEach-Object { $_.FullName.Substring('TH09-AI/source/src/ai/'.Length) } | Sort-Object)
